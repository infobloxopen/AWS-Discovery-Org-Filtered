...
- Least-privilege inline policy scoped to IPAM/network discovery
- EC2, VPC, IPAM, S3, Route53, ELB, DirectConnect discovery permissions
...
6. Optionally configure account filtering.
7. Create the stack.
...
| Service | Permissions |
|---------|------------|
| EC2 | `Describe*` for VPCs, Subnets, Instances, NAT/Internet/VPN Gateways, Transit Gateways, Peering, Endpoints, Security Groups, Route Tables, Network Interfaces, Volumes, Addresses |
| IPAM | `DescribeIpams`, `DescribeIpamScopes`, `DescribeIpamPools`, `GetIpamPoolAllocations`, `GetIpamPoolCidrs` |
...
| S3 | `ListAllMyBuckets`, `GetBucketPolicy`, `GetBucketPublicAccessBlock` |
